...
public class ItemDatabase {
	...
	static public void fillDatabase() {
		...
		dbItemList = mp.getAllItems();
		itemList = new ArrayList<com.netbuilder.test.Item>();

		
		for (int i = 0; i < dbItemList.size(); i++) { 
			
			System.out.println(dbItemList.get(0).getIdItem());
			
			itemList.add(icNothing.convertToWebItem((dbItemList.get(i))));
		}		
